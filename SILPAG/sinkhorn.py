import torch
"""
https://github.com/marvin-eisenberger/implicit-sinkhorn/tree/main
"""
...
class Sinkhorn(torch.autograd.Function):
    ...
    def forward(ctx, c, a, b, num_sink, lambd_sink):
        ...
        # log_b = torch.log(b).unsqueeze(dim=-2)
        epsilon = 1e-6
        log_a = torch.log(a + epsilon).unsqueeze(dim=-1)
        log_b = torch.log(b + epsilon).unsqueeze(dim=-2)
        for _ in range(num_sink):
    ...
    def backward(ctx, grad_p):
        ...
        if torch.isnan(K).any() or torch.isinf(K).any():
            print("!!! Matrix K contains NaN or Inf values !!!")
        epsilon = 1e-6
        K_reg = K + epsilon * torch.eye(K.shape[-1], device=K.device, dtype=K.dtype)
        # grad_ab = torch.linalg.solve(K_reg, t)
        grad_ab = torch.linalg.lstsq(K_reg, t).solution
        grad_a = grad_ab[..., :m, :]
